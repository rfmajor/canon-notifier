...
const handlers = {
    canon: {
        ...
        availabilityCheck: async (page) => {
            const availabilityElement = await page.$(
                "[data-testid=sf-stock-messaging-static]",
            );
    ...
    fotoplus: {
        ...
        availabilityCheck: (data) => {
            for (const keyword of ["SKLEP INTERNETOWY", "KRAKÓW", "KATOWICE"]) {
                ...
                const endIndex =
                    startIndex + str.substring(startIndex).indexOf("\n");
    ...
    fotoforma: {
        ...
        availabilityCheck: (data) => {
            ...
            const availabilityDivSubstring = data.substring(
                startIndex,
                endIndex,
            );
...
function groupRequestByType(
    ...
    if (handler["headless"]) {
        syncRequests.push(
            async () => await checkSite(siteName, siteUrl, browser),
        );
    } else {
...
async function checkSite(siteName, siteUrl, browser) {
    ...
    try {
        ...
        const handler = handlers[siteName];
        if (!handler["headless"]) {
            [available, error] = await checkNormal(
                siteName,
                siteUrl,
                handler["contentCheck"],
                handler["availabilityCheck"],
            );
        } else {
            [available, error] = await checkHeadless(
                siteName,
                siteUrl,
                handler["contentCheck"],
                handler["availabilityCheck"],
...
async function checkNormal(siteName, url, contentCheck, availabilityCheck) {
    try {
        return await withTimeout(async () => {
            ...
            if (!response.ok) {
                throw new Error(
                    `Invalid response status for ${siteName}: ${response.status}`,
                );
            }
            const data = await response.text();
            if (!contentCheck(data)) {
                throw new Error(
                    `No content loaded for ${siteName}, you might need to use a headless browser`,
                );
            }
    ...
    try {
        return await withTimeout(async () => {
            ...
            if (!(await contentCheck(page))) {
                throw Error(
                    `No content loaded for ${siteName}, it might be blocked by captcha`,
                );
            }
