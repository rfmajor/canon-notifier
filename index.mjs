...
const [twilioApiKey, accountSid] = await getSecrets(
    config.twilio.secretName,
    ...config.twilio.secrets,
);
const [sendGridApiKey] = await getSecrets(
    config.sendGrid.secretName,
...
async function runJob() {
    ...
    logger.info(
        `Last sent SMS messages: ${JSON.stringify(lastSentSmsMessages)}`,
    );
    ...
    for (let [siteId, lastSentDate] of Object.entries(lastSentSmsMessages)) {
        ...
        logger.info(
            `${siteName} is available, last SMS was sent on ${lastSentDate} (${parseInt(timeDiff / 1000 / 60 / 60)} hours ago)`,
        );
        const minIntervalHours =
            siteName in minSmsIntervalsHours
                ? minSmsIntervalsHours[siteName]
                : 1;
    ...
    try {
        await sendSMSMessage(
            accountSid,
            twilioApiKey,
            messageEligibleSites,
            config.sms.recipients,
            config.sms.messagingServiceId,
        );
...
cron.schedule(
    config.job.schedule,
    async () => await withTimeout(runJob, config.job.timeoutMs),
);
